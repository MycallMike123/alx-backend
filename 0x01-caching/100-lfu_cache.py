...
"""
LFU cache module that inherits from BaseCaching and is a caching
system Must use self.cache_data - dictionary from the parent class
BaseCaching LFU algorithm must be used to manage the cache
"""
...
class LFUCache(BaseCaching):
    """
    LFUCache class that inherits from BaseCaching Uses
    LFU algorithm to manage cache and swicthes to LRU
    algorithm if more than one item to discard.
    """
    ...
    def put(self, key, item):
        ...
        than BaseCaching.MAX_ITEMS:
            1) discard the least frequently used item in self.cache_keys
            2) discard the least frequently used item in self.cache_data
        """
    ...
    def get(self, key):
        """
        Return value in self.cache_data linked to key If key
        is None or key does not exist in self.cache_data, return None
        """
        if key is not None:
            if key in self.cache_data:  # if key exists in cachedata
